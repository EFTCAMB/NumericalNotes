...
\title{EFTCAMB/EFTCosmoMC: Numerical Notes v2.0}
%
...
The present version is based on EFTCAMB/EFTCosmoMC$\_$Oct15.
\end{abstract}
...
\begin{figure}[!tp]
\begin{center}
 \includegraphics[width=0.85\textwidth]{EFTCAMB_model_flags_V2}
...
%
In order to implement the EFT formalism in the CAMB and CosmoMC codes we had to modify several files. We list them in this Section to help the user  keep track of what the EFT code is doing and how. \\
...
%
\subsection{Structure of the EFTCAMB modification} \label{Sec:EFTCAMBStruct}
%
The modification to the CAMB code is mainly implemented by modifying or adding the following files:
%
\begin{itemize}
\item \verb|EFT_def.f90|: this file contains all the compile time EFTCAMB options and the definition of the parametrizations for the dark energy equation of state.
\item \verb|EFT_designer.f90|: this file contains the module for designer models. 
\item \verb|EFT_Horndeski.f90|: this file contains all the code regarding implementation of Horndeski models.
\item \verb|EFT_functions.f90|: this file contains the definition of the EFT functions for the chosen model, in particular it contains the definitions of the \textit{pure} EFT parametrizations.
\item \verb|EFT_main.f90|: this file contains a module that evaluates whether the considered DE/MG model is stable or not and a module that finds the time (if it exists) at which the considered model is so close to GR that it is pointless to evolve  the DE equation.
\item \verb|EFTstabilitySpace.f90|: this file contains a program that can be compiled with the directive \texttt{make eftstability} to serve the purpose of making simple explorations of the stability region in parameter space for the theory of interest. This proves extremely helpful to understand and visualize the shape of the parameter space of a theory before exploring it with CosmoMC. For a direct application of this see Figure (1) in~\cite{Raveri:2014cka}.
\item \verb|equations_EFT.f90|: this is a modified version of the standard CAMB equations file. It contains all the equations that the code needs to solve to get the full behaviour of perturbations in DE/MG models. These equations are reported in Section~\ref{Sec:ModEquations} and being written in terms of the EFT functions there is no need for the user to modify this file to include new DE/MG models in CAMB. 
\item \verb|cmbmain.f90|: it includes modification of the standard CAMB file to run the designer code, the stability check and the return to GR detection just after EFTCAMB is launched. It also contains an optional code that will print the behaviour of perturbations in the DE/MG model that is considered. The latter part of code is not controlled by the parameter file so the user has to search for it in the code and manually activate it. It is also possible to use it at debug purposes.
\end{itemize}
%
Several other files have to be marginally modified to complete the integration between CAMB and EFTCAMB. We encourage the user to inspect the released code for more details.

\subsection{Structure of the EFTCosmoMC modification} \label{Sec:EFTCosmoMCStruct}
To interface CosmoMC with EFTCAMB we had to modify the following files:
%
\begin{itemize}
\item \verb|params_CMB_EFT.ini|: to specify the EFT model parameters that are sampled in the MCMC run and their priors. 
\item \verb|common_batch1_EFT.ini| and \verb|common_batch2_EFT.ini|: to read cosmological parameters from \verb|params_CMB_EFT.ini|.
\item \verb|EFT_params.ini|: to let the user choose the model via the selection flags.
\item \verb|params_CMB.paramnames|: to add the names of the EFT model parameters.
\item \verb|Makefile|: to compile the whole program with the EFTCAMB/EFTCosmoMC files.
\item \verb|settings.f90|: to increase the number of theory parameters.
\item \verb|CosmologyTypes.f90|: to add the EFT model parameters into the set of CosmoMC parameters and read the values of the model selection flags.
\item \verb|CalcLike_Cosmology.f90|: to perform the stability check on the considered model for the specific choice of cosmological parameters that are being considered by the MCMC sampler and reject them if the theory is found to be unstable.
\item \verb|Calculator_CAMB.f90|: to pass the EFT model parameters and the values of the model selection flags to EFTCAMB.
\item \verb|Calculator_Cosmology.f90|: to make the stability module works with the newest version of CosmoMC.
\item \verb|CosmologyParameterizations.f90|: to increase the number of CosmoMC parameters and associate the EFT parameter name to the one in the parameters files.
\end{itemize}
%

As of now the EFTCosmoMC$\texttt{\_}$Oct15 code does not enforce automatically the compatibility between the model selection flags and the parameters that are included in the MCMC run. This means that the user has to do the following to properly launch a run:
%
\begin{itemize}
\item At first the proper values for the EFT model selection flags have to be chosen in the file \verb|EFT_params.ini| according to the scheme in Figure~\ref{Fig:StructureFlowChart}.
\item Then the user has to modify \verb|params_CMB_EFT.ini| to select the parameters to include in the run, their center values and priors.\\
In order to exclude unwanted EFT parameters from the run, their \texttt{start width} and \texttt{propose width} have to be set to zero while for the parameters related to the previously selected model these two have to be chosen different from zero.\\
Since EFTCAMB enforces \textit{viability priors} it is then sufficient that the standard CosmoMC priors are set to a value that reasonably includes the stable region in parameter space. \\
We then suggest to set the center of a parameter relatively far from its GR limit as the parameter space of a given model might be not well behaved in the vicinity of that point.
\item After these two steps the run can be launched in the standard way.
\end{itemize}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Background} \label{SubSec:EFTDesigner}
...
%
\frac{\dot{\Lambda} a^2}{m_0^2} =& -2\Omega\left(\ddot{\mathcal{H}}- \mathcal{H} \dot{\mathcal{H}} -\mathcal{H}^3 \right) - a^2\Omega^{\prime\prime}\mathcal{H}\left(2\mathcal{H}^2+3\dot{\mathcal{H}}\right)-a^3\mathcal{H}^3\Omega^{\prime\prime\prime} \nonumber\\
        & -a\Omega^{\prime}\left(4\mathcal{H}\dot{\mathcal{H}}+\ddot{\mathcal{H}}\right) + \frac{\rho_{\rm DE} a^2}{m_0^2} \mathcal{H}\bigg[a w^\prime_{\rm DE} -3 w_{\rm DE}(1+w_{\rm DE}) \bigg] \, .& & [\text{Mpc}^{-3}]
\end{align}
...
& k\dot{\eta} = \textcolor{red}{\f{1}{X}}\l[\frac{1}{1+\Omega} \frac{a^2(\rho_{m,\nu}+P_{m,\nu})}{m_0^2}\frac{v_{m,\nu}}{2} + \frac{k^2}{3H_0}F \textcolor{red}{+(U-X)\f{\mathcal{Z}k^2}{3}}\r]   \label{Eq:EtaEquation}\,,
\end{align}
%
